...
if ( ! class_exists( 'Construction_Landing_Page_WebFont_Loader' ) ) {
	/**
	 * Download webfonts locally.
	 */
	class Construction_Landing_Page_WebFont_Loader {
		...
		public function preload_local_fonts() {
			...
			update_site_option( 'construction_landing_page_local_font_files', $local_font );
		...
		public function get_base_path() {
			if ( ! $this->base_path ) {
				$this->base_path = apply_filters( 'construction_landing_page_get_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
			}
		...
		public function get_base_url() {
			if ( ! $this->base_url ) {
				$this->base_url = apply_filters( 'construction_landing_page_get_local_fonts_base_url', content_url() );
			}
		...
		public function get_subfolder_name() {
			if ( ! $this->subfolder_name ) {
				$this->subfolder_name = apply_filters( 'construction_landing_page_get_local_fonts_subfolder_name', 'fonts' );
			}
		...
		public function delete_fonts_folder() {
			...
			delete_site_option( 'construction_landing_page_local_font_files' );
...
if ( ! function_exists( 'construction_landing_page_get_webfont_styles' ) ) {
	/**
	 ...
	 */
	function construction_landing_page_get_webfont_styles( $url, $format = 'woff2' ) {
		$font = new Construction_Landing_Page_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_styles();
	}
}

if ( ! function_exists( 'construction_landing_page_get_webfont_url' ) ) {
	/**
	 ...
	 * @return string Returns the CSS.
	 */
	function construction_landing_page_get_webfont_url( $url, $format = 'woff2' ) {
		$font = new Construction_Landing_Page_WebFont_Loader( $url );
		$font->set_font_format( $format );
		return $font->get_url();
	}
...
/**
 * Create instance of Construction_Landing_Page_WebFont_Loader class.
 */
function construction_landing_page_webfont_loader_instance( $font_url = '' ) {
	return new Construction_Landing_Page_WebFont_Loader( $font_url );
}
